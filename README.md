...
このツールは以下の機能を持っています。
- **入力**: 作品情報や学生情報が記載されたCSVファイル（専用のgoogleformの集計結果をcsvファイルとして出力したもの）
- **処理**:
...
## 7. 技術的詳細・開発者向けマニュアル (Technical Manual)

以下は、コードを修正・拡張する開発者（メンテナンス担当者）向けの技術解説です。

### 7.1 コード構成 (Code Structure)
...
名前とふりがなを配置する関数です。
- 以下の特徴的な処理を行っています：
    - **1文字ごとの座標計算**: 単純な文字列描画ではなく、ループで1文字ずつ位置を計算しています。これにより、特定の文字（「ょ」「っ」など）ごとの微調整（`adjustments`）を可能にしています。
    - **縦書き回転**: アルファベットや特定の記号（`ROTATED_CHARS`）は、270度回転させて描画しています。
